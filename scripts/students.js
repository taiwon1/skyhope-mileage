...
import { showAlert, toLocalDate, validName, listenerError } from "./utils.js?v=20260917-1";
...
async function add() {
  ...
  try {
    await runTransaction(db, async tx => {
      ...
      if ((await tx.get(ref)).exists()) throw Error('이미 있는 학생입니다');
...
async function remove(name) {
  ...
  if (!confirm(msg)) return;
    ...
    if (!window.authState.isAdmin) return;
    // Keep the student until all activity batches have succeeded, so failed deletion can be retried.
    const docs = [...recSnap.docs];
    while (docs.length > 400) {
      const chunk = writeBatch(db);
      docs.splice(0,400).forEach(d=>chunk.delete(d.ref));
      await chunk.commit();
      if (!window.authState.isAdmin) return;
    }
...
function render() {
  ...
  // 한 달 미출석 판단
  const oneMonthAgo = kstToday();
  oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);
  const oneMonthAgoStr = toLocalDate(oneMonthAgo);
...
export function updateNameSelect() {
  ...
  if (recSel) {
    const selected = recSel.value;
    recSel.innerHTML =
      '<option value="">전체</option>' +
      studentList
        .map((s) => `<option value="${s.name}">${s.name}</option>`)
        .join("");
    recSel.value = selected;
  }
...
export function startListener() {
  onSnapshot(collection(db, "students"), (snap) => {
    ...
    render();
    updateNameSelect();
    window.newcomer?.populateGuideSelect();
    window.records?.resetToPage1();
    if (document.getElementById('tab-attend').classList.contains('active')) window.attendance?.render();
    if (document.getElementById('tab-summary').classList.contains('active')) window.summary?.render();
    if (document.getElementById('tab-ranking').classList.contains('active')) window.ranking?.render();
